export function generateThread(current: any, trendText: string) {
  ...
  if (trendText.includes("increased")) {
    thread += "1/ Failure rate is rising on testnet… here’s what’s happening 👇\n\n";
  } else if (trendText.includes("decreased")) {
    thread += "1/ Testnet just stabilized… but here’s what changed 👇\n\n";
  } else {
    thread += "1/ Testnet looks stable… but there’s a hidden signal 👇\n\n";
  }
  ...
  // Tweet 3
  thread += `3/ Only ${current.totalFailed} failed transaction(s) — sounds fine, right?\n\n`;
  ...
  if (current.totalFailed > 0) {
    ...
    thread += "4/ No failures detected — network looks clean.\n\n";
  }
